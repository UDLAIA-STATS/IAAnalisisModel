...
class CameraMovementEstimator():
    ...
    def get_camera_movement(
        ...
        for frame_num in range(1, len(frames)):
            ...
            camera_movement[frame_num], old_features = self.update_camera_movement(
                camera_movement, frame_num, camera_movement_x, camera_movement_y, max_distance, frame_gray)
    ...
    def update_camera_distance(self, new_features, old_features) -> tuple[int, int, int]:
        camera_movement_x, camera_movement_y, max_distance = 0, 0, 0
    ...
    def update_camera_movement(
            self, camera_movement, frame_num, 
            camera_movement_x, camera_movement_y, 
            max_distance, frame_gray):
        if max_distance > self.minimum_distance:
            camera_movement[frame_num] = [camera_movement_x, camera_movement_y] #inicializado fuera del for
            old_features = cv2.goodFeaturesToTrack(frame_gray,**self.features) # Inicializado fuera del for
        return camera_movement, old_features 
