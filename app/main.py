...
import numpy as np
from layers.infraestructure.validation import (calculate_interpolation_error,
                                               check_speed_consistency)
from layers.infraestructure.video_analysis.camera_movement_estimator import \
    CameraMovementEstimator
from layers.infraestructure.video_analysis.player_ball_assigner import \
    PlayerBallAssigner
from layers.infraestructure.video_analysis.plotting import generate_diagrams
from layers.infraestructure.video_analysis.services import (read_video,
                                                            save_video)
from layers.infraestructure.video_analysis.speed_and_distance_estimator \
    import SpeedAndDistance_Estimator
from layers.infraestructure.video_analysis.team_assigner import TeamAssigner
from layers.infraestructure.video_analysis.trackers.entities import (
    BallTracker, PlayerTracker)
from layers.infraestructure.video_analysis.trackers.services import \
    TrackerService
from layers.infraestructure.video_analysis.view_transformer import \
    ViewTransformer
...
def main():
    ...
    print(f"Detección de balón: {metrics['ball_detection']['detected']} frames "
          ...
          len(original_ball_tracks) * 100:.1f}%)")
