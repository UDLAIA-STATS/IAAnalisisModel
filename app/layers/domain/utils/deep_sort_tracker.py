...
class DeepSortTracker:
    ...
    def calculate_iou(self, bbox1: List[float], bbox2: List[float]) -> float:
        ...
        # Área de unión
        area1 = (x2 - x1) * (y2 - y1)
        area2 = (secondCoords.x2 - secondCoords.x1) * (secondCoords.y2 - secondCoords.y1)
    ...
    def update_tracks(self, players: List[Player]) -> List[Player]:
        ...
        for player in players: 
            ...
            for track in self.tracks:
                if track.team_id == player.team_id:
                    iou = self._calculate_iou(player.bbox, track.bbox)
                    distance = self._calculate_distance(player.position, track.position)
