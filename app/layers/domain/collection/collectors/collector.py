...
class Collector():
    """
    Clase principal que centraliza toda la lógica de comportamiento y almacenamiento.
    ...
    def add_frame_data(self, frame_number: int, teams_data: Dict[int, Dict[str, Any]], 
                      timestamp: float = 0.0) -> FrameCollector:
        """
        Añade datos de un frame con equipos y jugadores
        teams_data: {team_id: {'team_name': str, 'players': [player_data_dict]}}
        """
        # Crear frame
    ...
    def _maintain_frame_limit(self) -> None:
        ...
        if len(self.frames) > self.max_frames:
            ...
            self.frames = sorted(self.frames, key=lambda f: f.frame)[-self.max_frames:]
    
    def get_frame_by_number(self, frame_number: int) -> Optional[FrameCollector]:
    ...
    def get_frames_sorted(self, descending: bool = False) -> List[FrameCollector]:
        """Obtiene frames ordenados por número"""
        return sorted(self.frames, key=lambda f: f.frame, reverse=descending)
    
    def get_frames_in_range(self, start_frame: Optional[int] = None, 
    ...
    def get_team_by_id(self, team_id: int, frame_number: Optional[int] = None) -> Optional[Team]:
        """
    ...
    def get_top_players_by_speed(self, limit: int = 10) -> List[Player]:
        ...
        valid_players = [p for p in self.get_all_players() if p.speed is not None]
        return sorted(valid_players, key=lambda p: p.speed if p.speed is not None else 0.0, reverse=True)[:limit]
    
    def get_top_players_by_distance(self, limit: int = 10) -> List[Player]:
        """Obtiene los jugadores que más distancia han recorrido"""
        valid_players = [p for p in self.get_all_players() if p.distance is not None]
        return sorted(valid_players, key=lambda p: p.distance if p.distance is not None else 0.0, reverse=True)[:limit]
    ...
    def get_frame_evolution(self, start_frame: Optional[int] = None, 
                           end_frame: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    def set_frame_limit(self, limit: int) -> None:
        """Establece un nuevo límite de frames y aplica la limpieza si es necesario"""
