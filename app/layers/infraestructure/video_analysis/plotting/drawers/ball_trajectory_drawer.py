...
class BallTrajectoryDrawer(Diagram):
    ...
    def _split_trajectory_into_segments(
    ...
    def _draw_summary_diagram(self) -> None:
        ...
        plt.title(
            'Trayectoria Principal del Balón',
            color='white',
            fontsize=16)
    ...
    def _draw_table_diagram(self) -> None:
        ...
        for _, row in possession.iterrows():
            ...
            team_id = self.df[self.df['player_id']
                              == player_id].iloc[0]['team']
    ...
    def _draw_minimap(self, ax: Axes, segment: pd.DataFrame, idx: int) -> None:
        ...
        ax.scatter(
            ...
            color=color,
            edgecolor='white')
        ax.scatter(segment.iloc[-1].x, segment.iloc[-1].y,
                   s=30, color=color, marker='s', edgecolor='white')
